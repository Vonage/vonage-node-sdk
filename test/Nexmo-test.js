...
describe('Nexmo Object instance', function() {
    ...
    it('should have a default user agent in the form LIBRARY-NAME/LIBRARY-VERSION/LANGUAGE-VERSION', function() {
        var nexmo = new Nexmo({apiKey: 'test', apiSecret: 'test'});
        expect(nexmo.options.userAgent).to.match(/^nexmo-node\/[\d|\w\-\d.]* node\/[\d.]*$/);
    });
    ...
    it('should append to the user agent when a appendToUserAgent option is passed', function() {
        ...
        expect(nexmo.options.userAgent).to.match(/nexmo-node\/[\d|\w\-\d.]* node\/[\d.]* nexmo-cli\/1\.0\.0/);
    });
    ...
    it('should create same JWT as static function', () => {
        ...
        var nexmo = new Nexmo({apiKey: 'test', apiSecret: 'test', privateKey: privateKey, applicationId: appId});
