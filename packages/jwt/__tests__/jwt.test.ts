...
describe('Token Generator Method', () => {
    ...
    test('should throw if privateKey not a string or buffer', () => {
        expect(() =>
            (<any>tokenGenerate)(applicationId, 56789)
        ).toThrow('privateKey must be string or buffer')
    })
    ...
    test('should accept and use options', () => {
        let token = tokenGenerate(applicationId, privateKey, {ttl, subject, acl});
        let decoded = <any>(
            verify(token, privateKey, { algorithms: ['RS256'] })
        )
