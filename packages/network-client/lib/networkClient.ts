...
export class NetworkClient extends Client {
  ...
  constructor(auth: NetworkAuthParameters, config?: NetworkConfigParameters) {
    ...
    this.config.networkApiHost
      = config?.networkApiHost || 'https://api-eu.vonage.com';
  }
  ...
  async getODICRequest(): Promise<string> {
    try {
      ...
      if (error instanceof VetchError) {
        ...
        case 404:
          error.message
              = `It appears the network application is not setup correctly. `
              + `It is also possible that the phone number is not associated with this network. `;
          break;
        }
      }
