...
export class NetworkClient extends Client {
  ...
  constructor(auth: NetworkAuthParameters, config?: NetworkConfigParameters) {
    ...
    this.config = {
      ...
      storeToken: config?.storeToken || true,
  ...
  /**
   * Check if the token is is expired
   */
  ...
  async addAuthenticationToRequest(
    ...
    if (
      ...
      && this.authType === AuthenticationType.CIBA
      && (!this.accessToken || !this.isExpired)
    ) {
  ...
  buildOIDCURL(state: string = ''): string {
    ...
    const odicURL = new URL(`${this.config.networkApiHost}/oauth2/auth`);
  ...
  async exchangeCodeForToken(code: string): Promise<NetworkTokenResponse> {
    try {
      ...
      if (this.config.storeToken) {
        this.accessToken = response.data.access_token;
        this.expires = new Date().getTime() * 1000 + response.data.expires_in;
      }
   ...
   */
  async getCIBAToken(): Promise<void> {
    log('Getting token');
    // Turn off adding auth headers to the request while we get the token
    this.gettingToken = true;

    const cibaRequestId = await this.getCIBARequest();
   ...
   */
  async getCIBARequest(): Promise<string> {
    try {
      log(`Making CIBA request for ${this.msisdn}`);
