...
export const words = (str: string): Array<string> => str
  .split(/[-_\s]+/)
  .flatMap(word => word.split(/(?<=[a-z])(?=[A-Z])|(?<=[A-Z])(?=[A-Z][a-z])/))
  .filter(Boolean);

export const camelCase = (str: string): string => words(str)
...
export const snakeCase = (str: string): string => words(str)
  .map((word) => word.toLowerCase())
  .join('_');

export const kebabCase = (str: string): string => words(str)
  .map((word) => word.toLowerCase())
  .join('-');
