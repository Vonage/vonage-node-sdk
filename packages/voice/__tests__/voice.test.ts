...
import fs from  'fs';
import { NCCOBuilder, OutboundCallWithAnswerURL, Talk, Voice, OutboundCallWithNCCO } from '../lib/index';
import { BASE_URL } from '../lib/voice';
...
describe('voice', () => {
  ...
  test("can get call information", async () => {
    ...
    const results = await client.getCall("63f61863-4a51-4f6b-86e1-46edebcf9356");
  ...
  test("can get filtered call list", async () => {
    ...
    const results = await client.search({dateStart: '2019-12-31', dateEnd: '2020-01-02'});
    expect(results.count).toEqual(100);
  });
  ...
  test("can hang up call", async () => {
    nock(BASE_URL, {reqheaders: {'Authorization': value => value.startsWith('Bearer ') && value.length > 10 }})
      .persist()
      .put('/v1/calls/63f61863-4a51-4f6b-86e1-46edebcf9356', { action: "hangup" })
      .reply(204);
  ...
  test("can mute call", async () => {
    nock(BASE_URL, {reqheaders: {'Authorization': value => value.startsWith('Bearer ') && value.length > 10 }})
      .persist()
      .put('/v1/calls/63f61863-4a51-4f6b-86e1-46edebcf9356', { action: "mute" })
      .reply(204);
  ...
  test("can unmute call", async () => {
    nock(BASE_URL, {reqheaders: {'Authorization': value => value.startsWith('Bearer ') && value.length > 10 }})
      .persist()
      .put('/v1/calls/63f61863-4a51-4f6b-86e1-46edebcf9356', { action: "unmute" })
      .reply(204);
  ...
  test("can earmuff call", async () => {
    nock(BASE_URL, {reqheaders: {'Authorization': value => value.startsWith('Bearer ') && value.length > 10 }})
      .persist()
      .put('/v1/calls/63f61863-4a51-4f6b-86e1-46edebcf9356', { action: "earmuff" })
      .reply(204);
  ...
  test("can unearmuff call", async () => {
    nock(BASE_URL, {reqheaders: {'Authorization': value => value.startsWith('Bearer ') && value.length > 10 }})
      .persist()
      .put('/v1/calls/63f61863-4a51-4f6b-86e1-46edebcf9356', { action: "unearmuff" })
      .reply(204);
  ...
  test("can transfer call with URL", async () => {
    ...
    await client.transferCallWithURL("63f61863-4a51-4f6b-86e1-46edebcf9356", "https://example.com/ncco.json");
  });
  ...
  test("can transfer call with NCCO", async () => {
    ...
    await client.transferCallWithNCCO("63f61863-4a51-4f6b-86e1-46edebcf9356", builder.build());
  });
  ...
  test("can make a call with an NCCO", async () => {
    const expectedBody = {
        "ncco": [
          { action: 'talk', text: 'This is a call from Vonage'}
        ],
        "to": [
          {
            "type": "phone",
  ...
  test("can stream audio into a call", async () => {
    ...
    nock(BASE_URL, {reqheaders: {'Authorization': value => value.startsWith('Bearer ') && value.length > 10 }})
      .persist()
      .put('/v1/calls/63f61863-4a51-4f6b-86e1-46edebcf9356/stream', expectedBody)
      .reply(200, expectedResponse);

    const results = await client.streamAudio("63f61863-4a51-4f6b-86e1-46edebcf9356", "https://example.com/audio.mp3", 1, 0.4);
    expect(results.uuid).toEqual("63f61863-4a51-4f6b-86e1-46edebcf9356");
  ...
  test("can stop audio stream in a call", async () => {
    ...
    const results = await client.stopStreamAudio("63f61863-4a51-4f6b-86e1-46edebcf9356");
    expect(results.uuid).toEqual("63f61863-4a51-4f6b-86e1-46edebcf9356");
  ...
  test("can stream TTS into a call", async () => {
    ...
    const results = await client.playTTS("63f61863-4a51-4f6b-86e1-46edebcf9356", new Talk("Hello. How are you today?"));
    expect(results.uuid).toEqual("63f61863-4a51-4f6b-86e1-46edebcf9356");
  ...
  test("can stop TTS stream in a call", async () => {
    ...
    const results = await client.stopTTS("63f61863-4a51-4f6b-86e1-46edebcf9356");
    expect(results.uuid).toEqual("63f61863-4a51-4f6b-86e1-46edebcf9356");
  ...
  test("can stream DTMF into a call", async () => {
    ...
    const results = await client.playDTMF("63f61863-4a51-4f6b-86e1-46edebcf9356", "1713");
    expect(results.uuid).toEqual("63f61863-4a51-4f6b-86e1-46edebcf9356");
