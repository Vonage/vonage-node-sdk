...
export type PhoneEndpoint = {
  ...
  /**
   * Optional callback that is triggered when the call is answered before it is
   * joined to the conversation. Used with the Connect NCCO action.
   */
  onAnswer?: {
    /**
    ...
    /**
     * A URL value that points to a ringback tone to be played back on repeat
     * to the caller, so they do not hear just silence.
     */
    ringbackTone?: string;
