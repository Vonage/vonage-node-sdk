...
export class Record implements RecordAction {
    ...
    constructor(format?: RecordingFormat, split?: string, channels?: number, endOnSilence?: number, endOnKey?: string, timeout?: number, beepStart?: boolean, eventUrl?: string, eventMethod?: string){
        ...
        if (endOnSilence) { this.endOnSilence = endOnSilence }
        if (endOnKey) { this.endOnKey = endOnKey }
        if (timeout) { this.timeout = timeout }
        if (beepStart) { this.beepStart = beepStart }
    ...
    serializeToNCCO(): RecordAction {
        let data: RecordAction = {
            action: this.action
        }

        if (this.format) { data.format = this.format }
        if (this.wrappedSplit) { data.split = this.wrappedSplit }
        if (this.wrappedChannels) { data.channels = this.wrappedChannels }
        if (this.wrappedEndOnKey) { data.endOnKey = this.endOnKey }
        if (this.wrappedEndOnSilence) { data.endOnSilence = this.endOnSilence }
        if (this.wrappedTimeOut) { data.timeOut = this.wrappedTimeOut }
        if (this.beepStart) { data.beepStart = this.beepStart }
