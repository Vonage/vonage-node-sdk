...
export class Video extends Client {
    ...
    public async addArchiveStream(archiveId: string, streamId: string, audio = true, video = true): Promise<void> {
        const data = {
            addStream: streamId,
            hasAudio: audio,
            hasVideo: video,
    ...
    public async intiateSIPCall(sessionId: string, options: SIPCallOptions): Promise<SIPCallResponse> {
    ...
    public async playDTMF(sessionId: string, digits: string, connectionId?: string): Promise<void> {
