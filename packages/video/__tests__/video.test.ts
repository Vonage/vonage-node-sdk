...
const verifiyAuth = (value) => value.startsWith('Bearer ') && value.length > 10;

describe('video', () => {
    let client;

    beforeEach(() => {
        client = new Video(new Auth({
    ...
    test('can create a server session', async () => {
        nock(BASE_URL, { reqheaders: { 'Authorization': verifiyAuth } })
            .persist()
    ...
    test('can list all archives', async () => {
        ...
        expect(resp.items[0].sessionId)
            .toEqual(expectedResponse.items[0].sessionId);
        expect(resp.items[1].sessionId)
            .toEqual(expectedResponse.items[1].sessionId);
    });
    ...
    test('can add a stream to an archive', async () => {
        nock(BASE_URL, { reqheaders: { 'Authorization': verifiyAuth } })
            .persist()
            // eslint-disable-next-line max-len
            .patch('/v2/project/abcd-1234/archive/b40ef09b-3811-4726-b508-e41a0f96c68f/stream', {
                addStream: 'test-1234',
                hasAudio: false,
                hasVideo: true,
            })
    ...
    test('can mute streams', async () => {
        ...
        const resp = await client.forceMuteAll(
            'sess-1234',
            ['stream-pub-1234'],
        );
        expect(resp.id).toEqual(expectedResponse.id);
