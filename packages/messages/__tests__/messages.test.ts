...
describe('Messages', () => {
    ...
    test('can send using JWT auth', async () => {
        ...
        const results = await client.send(
            new SMS(expectedBody.text, expectedBody.to, expectedBody.from),
        );
        expect(results.message_uuid).toEqual(
    ...
    test.each`
        ...
        ${new TemplateMessage({
        name: 'MyNamespace:MyTemplate',
        parameters: [{ key: 'Bob Smith' }],
    }, '12225551234', '13335551234', 'en-GB')} | ${['template', 'whatsapp']}
        ${new CustomMessage({
        type: 'template',
        template: {},
    }, '12225551234', '13335551234')}                                                | ${'custom'}
    `(
        'can send various multimedia messages',
        async ({ messageObject, objectKey }) => {
            const expectedBody = {
                api_key: '12345',
                api_secret: 'ABCDE',
                to: messageObject.to,
