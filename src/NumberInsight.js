...
class NumberInsight {
    ...
    get(options) {
        ...
        if (level === 'advanced' || level === 'advancedAsync') {
            if (level === 'advanced') {
                console.warn('DEPRECATION WARNING: Number Insight Advanced with a level of "advanced" will be synchronous in v2.0+. Consider using the level "advancedAsync" to keep using the async option.');  // eslint-disable-line no-console
            }
