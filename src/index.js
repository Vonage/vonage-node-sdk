...
exports.getPhonePricing = function(product, msisdn, callback) {
    if (!product || (product != 'sms' && product != 'voice')) {
        ...
        pricingEndpoint.path += '/' + product + '/' + up.apiKey + '/' + up.apiSecret + '/' + msisdn;
        sendRequest(pricingEndpoint, callback);
    }
};
    ...
    if (!requestIds) {
        ...
        if (Array.isArray(requestIds)) {
            if (requestIds.length == 1) {
                requestIdParam.request_id = requestIds;  // eslint-disable-line camelcase
            } else {
                requestIdParam.request_ids = requestIds;  // eslint-disable-line camelcase
            }
        } else {
            requestIdParam.request_id = requestIds; // eslint-disable-line camelcase
        }
